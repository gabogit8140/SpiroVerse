...
import { SpiroConfig, Complex, GeneratorMode, SpiroType, ZModulationType } from '../types';
...
export const setupThreeScene = (width: number, height: number, container: HTMLElement) => {
    ...
    scene.fog = new THREE.FogExp2(0x020617, 0.00025); // Reduced fog density for better visibility at distance

    // High ambient light to simulate neon/self-illuminated look without washing out colors
    ...
    // Add a headlight attached to the camera to ensure the model is always lit from the view direction
...
export const updateTubeColors = (geometry: THREE.TubeGeometry, colors: number[], segments: number, radialSegments: number) => {
    const count = geometry.attributes.position.count;
    const colorAttr = new THREE.Float32BufferAttribute(new Float32Array(count * 3), 3);
    const colorCount = colors.length / 3;
    
    for (let i = 0; i <= segments; i++) {
        const cIdx = Math.min(Math.floor(i * (colorCount / (segments + 1))), colorCount - 1) * 3;
        const r = colors[cIdx];
        const g = colors[cIdx+1];
        const b = colors[cIdx+2];

        const vertsInRing = radialSegments + 1;
        for (let j = 0; j < vertsInRing; j++) {
            const vIdx = (i * vertsInRing + j) * 3;
            colorAttr.array[vIdx] = r;
            colorAttr.array[vIdx+1] = g;
            colorAttr.array[vIdx+2] = b;
        }
    }
...
export const buildMechanics = (
    ...
    // Tip Connector Line (Bridges gap between tube and pen)
